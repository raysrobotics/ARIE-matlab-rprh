% Use numerical method to calculate the intersactions
% 
...
% 
% Options:
%  - R: the radius of the peg and the hole (they are assumed to be of the same size)
...
%  2015-09-25 Fixed the bug when norm(zp) is small the result will be NaN.
...
m_div = 301;
n_div = 301;
x = linspace(-10,10,m_div);
y = linspace(-10,10,n_div);
[X, Y] = meshgrid(x,y);
...
for i = 1:length(x)
    for j = 1:length(y)
        ...
        [flag, lowest_point, ~] = ARIE_findLowestPoint_N(param, options, 0);
        
        if flag.outrange == 0 && isnan(lowest_point)
            ARIE_log(param, options, flag);
        else
            if flag.inter == Inf, flag.inter = 4; end
